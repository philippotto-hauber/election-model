...
dates_campaign <- load_dates_election_campaign(year = 2024, scen = "A")
```
...
prior_mmuT %>% tibble::rownames_to_column(var = "tmp") %>%
...
prior_ppi %>% as.data.frame() %>% tibble::rownames_to_column(var = "geography") %>% 
tidyr::pivot_longer(cols = -geography, names_to = "party", values_to = "value") -> df_prior_ppi
...
df_draws_ppi <- convert_draws_to_df(rstan::extract(out, pars = "ppi")[["ppi"]],                        
                            geographies = states,
                            parties = parties)
df_draws_ppi$t <- dates_campaign[df_draws_ppi$t]
```
...
plot_ppiT(df_draws_ppi, df_priors = df_prior_ppi, id_election_day, n_geographies = length(states), scen)
```
...
```{r}
plot_ppi(df_draws_ppi, filter(df_polls, scenario == scen), n_geographies = length(states), type_of_poll = "state", plt_title_prefix = scen)
```

```{r}
df_draws_ppi_reg <- convert_draws_to_df(rstan::extract(out, pars = "ppi_reg")[["ppi_reg"]],                        
                            geographies = regions,
                            parties = parties)
df_draws_ppi_reg$t <- dates_campaign[df_draws_ppi_reg$t]
```


```{r}
plot_ppi(df_draws_ppi_reg, filter(df_polls, scenario == scen), n_geographies = length(regions), type_of_poll = "regional", plt_title_prefix = scen)
```

```{r}
df_draws_ppi_nat <- convert_draws_to_df(rstan::extract(out, pars = "ppi_nat")[["ppi_nat"]],                        
                            geographies = "national",
                            parties = parties)
df_draws_ppi_nat$t <- dates_campaign[df_draws_ppi_nat$t]
```

```{r}
plot_ppi(df_draws_ppi_nat, filter(df_polls, scenario == scen), n_geographies = 1, type_of_poll = "national", plt_title_prefix = scen)
```