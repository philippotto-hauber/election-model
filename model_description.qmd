...
@Linzer_2013 and @Heidemanns_2020 as well as  @Stoetzer_etal_2019 are papers that my model is based on. 
...
To be able to model the dynamic evolution and correlation across parties and states more flexibly, the latent voting intentions are transformed to a vector of unbounded, real-valued quantities using the *additive log ratio transformation* (see XXX)

$$
\mu_{t, s} = \text{alr}(\pi_{t, s}) = \left( \frac{\pi_{t,s,1}}{\pi_{t,s,P}}, \dots, \frac{\pi_{t,s,P-1}}{\pi_{t,s,P}}\right)
$$

For each state $s$ this yields a vector of dimensions $P_s-1 \times 1$ where $P_s$ is the number of parties running in thatstate!
...
and $\mu^{\text{nat}}_{t} = \text{alr}(\pi^{\text{nat}}_{t})$.
...
The transformed voting intentions on election day in state $s$, $\mu_{T, s}$ are given a Normal prior centered on a *fundamental forecast* of the expected vote share in each state $\text{alr}(\pi^*_{t, s})$. All elements are assumed to be a priori independent but I allow for different prior variances $\sigma^2_s$ across states. By choosing a tighter prior more weight is placed on the fundamental forecasts. This may be reasonable in those states or regions where fewer polls are available.
...
The prior on the house effects is centered around 0 and the same prior variance $\sigma_{\delta}$ for all the elements: 
...
$$
\theta_{t, s, c} = \text{alr}^{-1}(\mu_{t, s} + \delta_{h})
$$

### State polls

The outcome of the $i = 1:I$ state polls are modeled as a realization of *multinomial random variable*

$$
y_i \sim \text{Multinomial}(N_i, \theta_{t[i], s[i], h[i]})
$$

where $N_i$ is the total number of respondents and $y_i = [y_{i, 1}, \dots, y_{i, P}]$ is a vector containing the number of respondents who in poll $i$ on day $t[i]$ would vote for the candidate of party $p$ in state $s[i]$.

### Regional polls

Indexed by $j=1:J$, regional polls are modeled in exactly the same way as state polls. The only difference is that underlying probabilities of the multinomial distribution are derived from the regional voting intentions, $\pi^{reg}_{t[j], p[j], r[j]}:

$$
y^{reg}_j \sim \text{Multinomial}(N_i, \pi^{reg}_{t[j], p[j], r[j]})
$$

### National polls

Similarly, national polls are 