...
# # store data in list
# polls[["n_responses"]]   <- df_polls_state$sample_size
# polls[["day_poll"]]      <- sapply(df_polls_state$date, 
#                                             function(x){ which(dates_campaign == x)})
# polls[["house_poll"]]    <- sapply(df_polls_state$pollster, 
#                                             function(x){which(pollsters == x)})
# polls[["state_poll"]]    <- sapply(df_polls_state$geography, 
#                                             function(x){which(states_regions$state == x)}) 
# polls[["y"]]             <- t(as.matrix(df_polls_state[, c("cc_n", "dgm_n", "pdal_n", "ssp_n")]))

# # regional polls ----
# dat %>%
#     filter(type_of_poll == "regional") %>%
#     arrange(date) -> df_polls_region  
...
# polls[["n_responses_reg"]]   <- df_polls_region$sample_size
# polls[["day_poll_reg"]]      <- sapply(df_polls_region$date, 
#                                                     function(x){ which(dates_campaign == x)})
# polls[["house_poll_reg"]]    <- sapply(df_polls_region$pollster, 
#                                                     function(x){which(pollsters == x)})
# polls[["region_poll"]]       <- sapply(df_polls_region$geography,
#                                                     function(x){which(unique(states_regions$region) == x)}) 
# polls[["y_reg"]]             <- t(as.matrix(df_polls_region[, c("cc_n", "dgm_n", "pdal_n", "ssp_n")]))

# # national polls ----
# dat %>%
#     filter(type_of_poll == "national") %>%
#     arrange(date) -> df_polls_national
...
# polls[["n_responses_nat"]]   <- df_polls_national$sample_size
# polls[["day_poll_nat"]]      <- sapply(df_polls_national$date, 
#                                                 function(x){ which(dates_campaign == x)})
# polls[["house_poll_nat"]]    <- sapply(df_polls_national$pollster, 
#                                                 function(x){which(pollsters == x)})
# polls[["y_nat"]]             <- t(as.matrix(df_polls_national[, c("cc_n", "dgm_n", "pdal_n", "ssp_n")]))
...
#     file = here::here(
#         "backtest",
#         paste0("df_polls_", params$year, ".csv")
#         ), 
#     row.names = FALSE
# )
