...
Set this value equal to Stoeltzer et al. (Note, however, that there is a discrepancy between i) the value set in the code - 0.001 - and the accompanying comment ("1 percent point sd") and ii) what the say about the prior in the original paper - !)

```{r}
sig_ddelta <- 0.001 # specify in terms of standard deviation for Stan!
```
...
scale_scenarios <- c(
    ...
    "B" = 1,  # very few polls, only in April, many regions and states without any
    "C" = 1,  # polls up until May, almost all regions and states covered but mostly national
    "D" = 1,    # similar to A
    "E" = 1   # similar to C
)
```
