...
for (mv in macro_vars) {
    for (s in contested_states) {
        ...
        while (ind_p <= n_parties_by_state[s]) {
            ...
            if (significant_macro_impact(tmp_lm, crit_val = 0.05)) {
                pred <- c()
                for (scen in scenarios) {
                    predict_vote_share(
                        tmp_lm,
...
Above I only checked whether macroeconomic fundamentals had a *statistically* significant effect on the election outcome. To gauge the *economic* significance, I compare the predicted vote share in the different scenarios with the historical average (ignoring those vote shares for which no statistically significant effects were found)

```{r}
df_fund_fcast %>% 
    mutate(
        diff_voteshare = pred - vote_share
    ) %>%
    ggplot(aes(x = diff_voteshare)) +
    geom_histogram(bindwidth = 0.001) +
    labs(x = "", title = "Difference between conditional forecast and historical average")
```
