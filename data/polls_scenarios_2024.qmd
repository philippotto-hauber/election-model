...
Function to convert polls to `data.frame` and plot for a given scenario

```{r}
gen_plots <- function(scen) {
    ...
    df_plt %>%
        ...
        theme(legend.position="top") -> plt_reg_nat
    
    return(list(plt_state = plt_state, plt_reg_nat = plt_reg_nat))
}
```
...
```{r}
#| echo: false
plts_tmp <- gen_plots("A")
plts_tmp$plt_state
```

```{r}
#| echo: false
plts_tmp$plt_reg_nat
```
