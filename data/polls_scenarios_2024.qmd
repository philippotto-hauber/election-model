...
### Date
 ...
 The publication lags of polls do not vary much and are at most two days so it probably does not make much of a difference which of the two variables - `date_conducted` or `date_published` is chosen as the "date" of a poll 
...
Since results reflect the voting intentions when the poll was *conducted*, set `date_conducted` to `date`.
...
ggplot(
 ...
 ggsave(file = here::here("plots", "polls_by_date.png"))   
```
...
ggplot(aes(x = geography, fill = type_of_poll)) +
    ...
    coord_flip()
ggsave(file = here::here("plots", "polls_by_type.png"))
```
...
ggplot(dat, 
 ...
 ggsave(file = here::here("plots", "polls_by_sponsor.png"))   
```
...
ggsave(file = here::here("plots", "polls_by_house.png"))   
```
...
ggsave(file = here::here("plots", "polls_by_mode.png"))   
```
...
ggsave(file = here::here("plots", "polls_by_population.png"))   
```
